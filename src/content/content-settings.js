(() => {
  ...
  class CommentHistoryExtension {
        ...
        try {
          ...
          if (storageData.commentedPosts) {
            ...
            if (Array.isArray(storageData.commentedPosts)) {
              ...
              this.commentedPosts = new Map(
                Object.entries(storageData.commentedPosts),
              );
            }
            this.debug(
              `Загружено ${this.commentedPosts.size} прокомментированных постов`,
            );
          } else {
            this.commentedPosts = new Map();
          }
    ...
    async cleanupOldCommentedPosts() {
      ...
      try {
        ...
        const maxAgeMs =
          CONFIG?.LIMITS?.COMMENTED_POSTS_MAX_AGE || 7 * 24 * 60 * 60 * 1000; // По умолчанию 7 дней
        const maxStorageLimit =
          CONFIG?.LIMITS?.MAX_COMMENTED_POSTS_STORAGE || 1000;
  ...
  const descriptors = Object.getOwnPropertyDescriptors(
    CommentHistoryExtension.prototype,
  );

  ...
  class LimitsExtension {
    async decrementFreeParsing(options = {}) {
      try {
        ...
        const storedExtKey =
          storageData[DECREMENT_PARSE_CONFIG.EXTENSION_KEY_STORAGE];

        const tg_id =
          typeof options?.tg_id === "number" && Number.isFinite(options.tg_id)
            ? options.tg_id
            : authData?.tg_id;
        ...
        if (
          !extension_key ||
          !EXTENSION_KEY_PATTERN.test(String(extension_key))
        ) {
        ...
        if (!responseData || typeof responseData !== "object") {
          return {
            success: false,
            error: responseText
              ? `invalid_body_http_${response.status}`
              : `empty_body_http_${response.status}`,
          };
        }

        if (response.ok && responseData?.success) {
          const parsingLeft = Number(responseData.parsing_left);
          const finalParsingLeft = Number.isFinite(parsingLeft)
            ? parsingLeft
            : 0;
          ...
          if (authData) {
            ...
            await chrome.storage.local.set({
              [DECREMENT_PARSE_CONFIG.AUTH_STORAGE_KEY]: authData,
            });
          }
        ...
        if (!response.ok) {
          return {
            success: false,
            error: errorMsg || `http_error_${response.status}`,
          };
        }

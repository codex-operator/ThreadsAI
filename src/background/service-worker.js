...
class ThreadsAIServiceWorker {
  ...
  setupEventListeners() {
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      ...
      return true; // держим канал открытым для async-ответов
    });
  ...
  async generateComment(postText, settings) {
    ...
    if (!openaiApiKey) {
      throw new Error('API-ключ OpenAI не указан. Откройте настройки расширения.');
    }

    const systemPrompt = PRESET_PROMPTS[commentStyle]?.prompt

    ...
    class SettingsExtension {
        ...
        async getInlineAISettings() {
            const [syncData, localData] = await Promise.all([
                ...
                chrome.storage.local.get([STORAGE_KEYS.AUTH, STORAGE_KEYS.EXTENSION_KEY, 'customPrompts'])
            ]);

            const authData = localData[STORAGE_KEYS.AUTH] || null;
            const extensionKey = localData[STORAGE_KEYS.EXTENSION_KEY] || null;
            const localCustomPrompts = Array.isArray(localData.customPrompts) ? localData.customPrompts : [];
            const syncCustomPrompts = Array.isArray(syncData.customPrompts) ? syncData.customPrompts : [];
            
            const customPrompts = localCustomPrompts.length ? localCustomPrompts : syncCustomPrompts;
            ...
            if (syncData.commentStyle === 'custom') {
                ...
                const selectedPrompt = customPrompts.find(p => p && p.id === selectedPromptId);
                instructions = selectedPrompt?.text || syncData.customPrompt || null;
            } else {
                instructions = PRESET_PROMPTS?.[syncData.commentStyle]?.prompt 
            ...
            if (!localCustomPrompts.length && syncCustomPrompts.length) {
                chrome.storage.local.set({ customPrompts: syncCustomPrompts }).catch(() => {});
            }
